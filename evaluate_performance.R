...
pred_constrain <- function(x, beta, ndraws){
  ...
  for (i in 1:nrow(x)){
    ...
    # D | A, Q
    yD_s = sample_norm(x_D[i,], beta_D, cov_D, ndraws)
    ...
    # M | Q, A, D
    # xR1 = do.call("rbind", replicate(ndraws^2, x_R1[i, ], simplify = FALSE))
    xM = t(matrix(rep(as.numeric(x_M[i, ]), ndraws^2), nrow = ncol(x_M)))
    xM[, 3] = rep(yD_s, ndraws)
    #xM[, 4] = yL_s
    yM_s = sample_norm(xM, beta_M, cov_M, ndraws)
